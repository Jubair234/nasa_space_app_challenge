...
const linkContent = document.getElementById("link_content");
button.addEventListener("click", () => {
  if (linkContent.classList.contains("hidden")) {
...
const loadData = () => {
  fetch(
    "http://api.open-notify.org/iss-now.json?fbclid=IwAR2060Q431PwsWDsO4Amgb_QVorai7I-ANN8Ys1ZMwA3xNoCWrQdqlVtzOM"
  )
    .then((res) => res.json())
...
const details = (info) => {
  console.log(info);
  const allinfo = document.getElementById("container");
  allinfo.innerHTML = "";
  const div = document.createElement("div");
  div.classList.add("innerDiv");
  div.innerHTML = `
        <h3>Traking :${info.message}</h3>
        <h3>ISS Position:${info.timestamp}</h3>
        <h3>latitude:${info.iss_position.latitude}</h3>
        <h3>longitude:${info.iss_position.longitude}
        </h3>
        <h3> Altitude:<span>431km</span> </h3>
        <h3> Speed:<span>27572</span>km/h</h3>
    `;
...
(async function () {
  ...
  let plane = (await new GLTFLoader().loadAsync("ISS-3D.glb")).scene
    .children[0];
...
function makePlane(planeMesh, trailTexture, envMap, scene) {
  ...
  plane.scale.set(0.02, 0.02, 0.001);
  plane.position.set(0, 0, 0);
  plane.rotation.set(0, 1, 0);
  plane.updateMatrixWorld();
